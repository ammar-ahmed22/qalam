      ...
      if let Some(Literal::Array(arr)) = arr {
        if let Some(Literal::Number(start)) = start {
          if let Some(Literal::Number(end)) = end {
            ...
            if !is_usize(**end) {
              return Err(RuntimeError::init(paren, format!("'end' must be a positive integer!")))
            }

            if end < start {
              return Err(RuntimeError::init(paren, format!("'end' must be greater than or equal to 'start'.")))
            }
            let slice = arr.0.as_ref().borrow().elements[(**start as usize)..(**end as usize)].to_vec();
            let new_arr = QalamArray::from_vec(slice);
            return Ok(Some(Literal::Array(HashableRcRefCell::init(new_arr))));
          } else {
            return Err(RuntimeError::init(paren, format!("'end' must be a number!")))
          }
        } else {
          return Err(RuntimeError::init(paren, format!("'start' must be a number!")))
        }
      } else {
