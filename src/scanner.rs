...
impl<'a> Scanner<'a> {
    ...
    fn string(&mut self) {
        ...
        if self.end() {
            self.error_reporter
                .error(self.line, "Unterminated string.", None, ErrorType::Syntax, None);
            return;
        }
        ...
        match value {
            Some(val) => self.add_token(TokenType::String, Some(Literal::String(val.to_string()))),
            None => {
    ...
    fn number(&mut self) {
        ...
        match value {
            Some(val) => {
                ...
                self.add_token(
                    TokenType::Number,
                    Some(Literal::Number(OrderedFloat(as_float))),
                );
            }
            None => {
    ...
    fn identifier(&mut self) {
        ...
        match self.source.get(self.start..self.current) {
            Some(text) => match TokenType::from_keyword(text) {
                Some(token_type) => {
